...
function PersonalConnectionSetup({ connection, refreshConnection }) {
  ...
  return <form className="form-stack" onSubmit={connect}>
    ...
    <p className="micro left">Your key is kept in this tab’s session storage, so refreshes keep the connection. Browser session restore may also keep it. Disconnect below to remove it. Each AI request passes it securely through Lexica to the provider above; it is not saved on the server, shared with other visitors, or included in backups. Testing uses a small amount of API credit.</p>
